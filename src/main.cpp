...
class Pomodoro {
 public:
  ...
  void setup() {
    ...
    _time_per_session =
        inputIntRequest("Specify time per session on Serial:" + std::to_string(SERIAL_PORT));
    _time_per_break =
        inputIntRequest("Specify time per break on Serial:" + std::to_string(SERIAL_PORT));
    _time_per_long_break =
        inputIntRequest("Specify time per long break on Serial:" + std::to_string(SERIAL_PORT));
  }
 ...
 private:
  ...
  void printFocusInfo() {
    std::string focus_info = "Focus!" + getRemainingTime();
    _lcd.setCursor(0, 0);
    _lcd.print(focus_info.c_str());
  }

  void printBreakInfo() {
    std::string break_info = "Break!" + getRemainingTime();
    _lcd.setCursor(0, 0);
